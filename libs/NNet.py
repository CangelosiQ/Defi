...
'loss','classification_loss','least_square_loss','L2_loss',
...
def back_propagation(model, X, y, wd_coefficient):
    ...
    for t in model.type_layer:
        ...
        i=i+1
    if model.type_layer[-1] is 'softmax':
        class_prob = np.exp(Z[-1])
        # hidden to output gradient
        output_delta = (class_prob - y)
    else:
        output_delta = (Z[-1] - y)*2
    dW=[]
    dB=[]
    cumul=1
...
def build_model(X, y, wd_coefficient, dim_model, type_layer, loss_type,
                   ...
                   print_info=False,train_test_split=True,X_test='',y_test=''):

    N=y.shape[1]
    mini_batch_size=mini_batch_coef*N
                   #    X=preprocessing.normalize(X,axis=0)
    print('N,mini_batch_size',N,mini_batch_size)
...
def least_square_loss(y,t):
    if type(y)==list:
        print('transforming list into np array')
        y=np.reshape(y,(len(y),))
        print(y.shape)
    if type(t)==list or t.shape!=y.shape:
